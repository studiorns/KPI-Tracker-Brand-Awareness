...
const LATEST_MONTH = 'December'; // Make sure this matches exactly with the month name in the CSV
...
function createActualVsForecastChart(canvasId, data, initiative, subInitiative, metric) {
  try {
    ...
    // Filter for months January through December (full year)
...
function createMonthlyTrendChart(canvasId, data, metric) {
  ...
  const months = MONTHS.slice(0, 12); // January through December (full year)
  const datasets = [];
  ...
  const chart = new Chart(ctx, {
    ...
    options: {
      ...
      plugins: {
        ...
        title: {
          display: true,
          text: `Monthly ${metric} Trend - Full Year 2025`,
          color: '#e2e8f0',
          font: {
            family: "'Inter', sans-serif",
            size: 16,
...
function createYTDAchievementChart(canvasId, data, ytdAchievement) {
  ...
  const datasets = [];
  const latestMonth = 'December'; // Latest month with actual data
