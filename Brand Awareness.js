// Brand Awareness Dashboard - Chart Configurations and Data Processing
// This file handles data processing and chart creation for the Brand Awareness KPI Dashboard

console.log('Brand Awareness.js loaded successfully');
...
const LATEST_MONTH = 'September'; // Make sure this matches exactly with the month name in the CSV
...
function createActualVsForecastChart(canvasId, data, initiative, subInitiative, metric) {
  try {
    ...
    // Filter for months January through September
    const displayMonths = ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September'];
    ...
    const subInitiativeColors = {
      'Brand Awareness T1': CHART_COLORS.blue,
      'Brand Awareness UAE': CHART_COLORS.green,
      'Brand Awareness T2': CHART_COLORS.orange
    };
...
function createMonthlyTrendChart(canvasId, data, metric) {
  ...
  const months = MONTHS.slice(0, 9); // January through September
  const datasets = [];
  ...
  const chart = new Chart(ctx, {
    ...
    options: {
      ...
      plugins: {
        ...
        title: {
          display: true,
          text: `Monthly ${metric} Trend - Jan-September 2025`,
          color: '#e2e8f0',
          font: {
            family: "'Inter', sans-serif",
            size: 16,
...
function createYTDAchievementChart(canvasId, data, ytdAchievement) {
  ...
  const datasets = [];
  const latestMonth = 'September'; // Latest month with actual data
    ...
    options: {
      ...
      scales: {
        ...
        y: {
          ...
          min: 0,
          max: 250, // Increased to accommodate high achievement values like 224.3%
          grid: {
